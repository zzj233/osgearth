...
DrapingDecorator::CameraLocal::initialize(osg::Camera* camera, DrapingDecorator& decorator)
{
    ...
    tex->setWrap(tex->WRAP_T, tex->CLAMP_TO_BORDER);
...
void DrapingDecorator::Cascade::computeProjection(const osg::Matrix& rttView, const osg::Matrix& iCamMVP, const osg::Plane& plane,
                                                  double dp, double maxExt, double prevMaxY)
{
    ...
    if (_minX == DBL_MAX || _minX < -maxExt) _minX = -maxExt;
    if (_maxX == -DBL_MAX || _maxX > maxExt) _maxX = maxExt;
    if (_minY == DBL_MAX || _minY < -maxExt) _minY = prevMaxY;
    if (_maxY == -DBL_MAX || _maxY > maxExt) _maxY = maxExt;
...
{    
    ...
    // "maxExt" is the distance from the eyepoint to the horizon distance, 
    // projected into the horizon plane. This the largest extent we will need for RTT.
    ...
    double maxExt = sqrt(dh*dh - dp*dp);
    ...
    // camera clip -> camera view
    osg::Matrix camMVInv;
    camMVInv.invert(camMV);
    ...
    _cascades[0].computeProjection(rttView, iCamMVP, horizonPlane, dp, maxExt, -maxExt);
    ...
    // method.
    {
        ...
        _cascades[1]._minClipY = -0.95; //_cascades[0]._maxClipY;
        _cascades[1]._maxClipY = -0.15;

        _cascades[2]._minClipY = -0.15;
        _cascades[2]._maxClipY =  0.35;

        _cascades[3]._minClipY =  0.35;
        _cascades[3]._maxClipY =  1.00;

        _numCascades =
            h >= 4.5 && _maxCascades >= 4 ? 4 :
            h >= 3.0 && _maxCascades >= 3 ? 3 :
            h >= 2.0 && _maxCascades >= 2 ? 2 : 1;

        _cascades[_numCascades-1]._maxClipY = 1.0;

        //if (camera->getName() == "dump")
        //    OE_WARN << "height = " << cascades[0].heightNDC << std::endl;
    }
    ...
    for (i = 0; i < _numCascades; ++i)
    {
        ...
        if (_numCascades > 1u)
        {
            double prevMaxY = i == 0 ? -maxExt : _cascades[i-1]._maxY;
        ...
        texMat.setElement(i, camMVInv * rttView * cascade._rttProj * clipToTex);
    }
