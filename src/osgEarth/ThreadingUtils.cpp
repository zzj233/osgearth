...
GateMutex::GateMutex(const std::string& name) :
    _m(name)
{
    //NOP
}

void GateMutex::lock(const void* t)
{
    std::unique_lock<Mutex> lock(_m);
    while(_users[t] > 0)
        _unlocked.wait(lock);
...
void GateMutex::unlock(const void* t)
{
    std::unique_lock<Mutex> lock(_m);
    _users[t]--;
    if (_users[t] == 0)
...
void GateMutex::setName(const std::string& name)
{
    _m.setName(name);
}
